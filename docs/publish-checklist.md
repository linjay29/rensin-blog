# 發布一篇文章的完整流程

一篇文章不是 push 上線就結束，網站只是落地頁。真正帶人進來的是 FB 貼文、
IG 貼文與 IG 限動這三個入口，而三個入口要的文案與圖檔規格都不一樣。
這份清單就是把「上線 → 生素材 → 三個平台發布」一次走完的順序。

以下用 `<slug>` 代表 `YYYYMMDD-短名`，例如 `20260903-fall-tailbone`。

---

## A0. 先給預覽網頁，不要貼純文字

- [ ] `node tools/serve.mjs`（或 Browser pane 的 preview_start，`.claude/launch.json` 已指到這支）
- [ ] 把草稿網址給傑哥：`http://localhost:4321/_draft-<slug>/`，**用手機寬度 375×812 開著**
- [ ] 他直接在預覽頁上看、指哪裡改。**不要把整篇文字貼在對話裡**——
      排版、斷行、圖的位置、手機上的樣子，純文字看不出來（2026-09-14 傑哥定）

## A. 文章上線

- [ ] **定稿**：草稿在 `public/_draft-<slug>/`，定稿後把資料夾改名成 `public/<slug>/`
      （`_draft-` 開頭的資料夾 build 不收，deploy 也會在打包前刪掉，網址打對也開不了）
- [ ] **封面圖**：`public/<slug>/assets/cover.jpg`
      首頁卡片會自動抓這個路徑，不用另外設定；沒有這張圖就是純文字卡。
      **一律用 `tools/cover-card.ps1` 產生，不要用社群圖的流程做封面**——
      那支腳本會一次疊上標題、湖綠短線與**右下角的診所 logo**，logo 是每張封面的固定元素：

      ```bash
      powershell -NoProfile -ExecutionPolicy Bypass -File tools/cover-card.ps1 -Src assets-src/<slug>/cover-src.jpg -Text assets-src/<slug>/cover-title.txt -Out public/<slug>/assets/cover.jpg -X 760 -Y 230 -Size 86
      ```

      - `cover-title.txt`（UTF-8，一行一列，通常兩行）就是壓在圖上的標題，**文字要跟文章標題一致**
      - `-X/-Y/-Size` 每張圖都要看著調：主體在右就把字放左邊，反之亦然。**調完一定把圖叫出來看一次**
      - `-Logo br`（預設）右下、`-Logo bl` 左下；主體壓到角落時才換邊
      - 要把字寫進插畫裡的招牌或顯示燈，用 `tools/cover-sign.ps1`（支援旋轉角度與顏色，
        例如 `-Text 重要資訊 -Angle 12 -Color "#EB6B89"`，粉紅取自 logo）
      - **換過封面就要更新 `<img src="assets/cover.jpg?v=…">` 的版本號**（首頁卡片由 build 自動加）；
        圖片的 CDN 快取是四小時，不加版本號會看到舊圖

- [ ] ⚠️ **封面與社群圖是兩套流程，不可混用**：封面 `cover-card.ps1`（帶 logo、1200 寬）、
      社群圖 `social-card.ps1`（方形 2048 與限動 1080×1920，見 B 段）。
      2026-09-13 曾因為拿社群圖當封面，導致〈手指也會長石頭？〉少了 logo
- [ ] **內文圖**：其餘配圖放同一個 `assets/`，在 HTML 裡用相對路徑引用
- [ ] **build 通過**

      ```bash
      node tools/build.mjs
      ```

      會重算更新日期、重建首頁卡片與 sitemap，並跑中文斷行檢查。
      有錯就會擋下來，不要繞過。

- [ ] **commit + push 到 main**，GitHub Actions 自動部署（約 40 秒）
- [ ] **驗證線上**

      ```bash
      curl -s -o /dev/null -w "%{http_code}\n" https://rensin-clinic.sclin.net/<slug>/
      ```

      文章頁要 200、首頁要看得到新卡片、其他草稿網址要 404

---

## B. 生社群素材

- [ ] **無字原圖**：Gemini 生成時一律要求 `No text, no lettering, no numbers`，
      中文一律由我們自己合成（AI 畫的中文字必壞）。原圖收進
      `assets-src/<slug>/cover-src.jpg`（`assets-src/` 不進版控、不部署）
- [ ] **圖裡有護理師時，臉型要像高淑麗**——她是傑哥的太太，也是跟診的護理師。
      參考照放在 `assets-src/staff/`，生圖時一併餵給 Gemini；不要用隨機長相。
      白袍是立領白色、胸前繡紫色「護理師 高淑麗」。仍標「圖為示意圖」。

- [ ] **重點文字檔**：`assets-src/<slug>/social-card.txt`，UTF-8，一行一項

      ```
      標題
      重點一（不超過 18 字）
      重點二
      重點三
      林士傑醫師
      ```

- [ ] **生圖**（一次產出方形與限動兩張）

      ```bash
      powershell -NoProfile -ExecutionPolicy Bypass -File tools/social-card.ps1 -Src assets-src/<slug>/cover-src.jpg -Text assets-src/<slug>/social-card.txt -Out assets-src/<slug>/social-fb.png
      ```

      產出 `social-fb.png`（2048×2048）與 `social-fb-story.png`（1080×1920）

- [ ] **文案**：`assets-src/<slug>/social.txt`，一個檔案裝三份——
      FB 貼文、FB 第一則留言、IG 貼文

---

## B1. IG：貼文與限動都要發，任務不同

| | 任務 | 圖 |
|---|---|---|
| **貼文** | 長尾——留在版面、被搜尋到、新追蹤的人往下滑會看到 | `social-fb.png`（2048×2048） |
| **限動** | 24 小時的提醒，**而且是 IG 唯一能點的連結入口** | `social-fb-story.png`（1080×1920） |

⚠️ **IG 貼文內文的網址點不了**，所以導流全靠限動的**連結貼紙**。
順序：發貼文 → 馬上發限動 → 限動加連結貼紙指到文章網址 → 把限動加進精選。

### 精選（Highlights）照網站的分類走

| 精選 | 放什麼 |
|---|---|
| **仁心百態** | 診間故事類的限動 |
| **仁心講骨** | 衛教類的限動 |
| **就診須知** | 掛號時間、營運公告、就診原則 |

**名字跟網站、粉專用同一組**——同一個系列在三個地方叫同一個名字，讀者才記得住。
新追蹤的人點進個人檔案，三個圈就知道這裡在做什麼。

## B2. 要不要轉到傑哥的個人臉書

**先看文章性質，再決定轉不轉**——不是每篇都轉，每篇都轉個人頁會變廣告牆。

| 分類 | 轉個人頁？ | 為什麼 |
|---|---|---|
| **仁心百態**（人的樣子、診間故事） | ✅ 轉 | 朋友追蹤的是他這個人，故事才有人看 |
| **仁心講骨**（病與治療） | ⚠️ 看情況 | 純衛教留在粉專；除非那篇有很強的故事開場 |
| **診所公告**（就診原則、營運公告） | ❌ 不轉 | 規則與行政訊息，個人頁轉了像在對朋友宣導 |

要轉的話，**交付素材時一起附一段兩行的個人頁短文案**（不是粉專那份）：

- 第一行放金句，第二行放鉤子，**兩行就好**
- **不要寫「今天」「昨天」**——他可能隔幾天才轉，時間字眼會對不上
- 不要寫成「某某教我的事」這種，事情本來就是他在做的（2026-09-14 他糾正過）
- 範例：「講清楚，聽明白，好的快。／一位阿嬤的掛號烏龍，還有那天真正難的事。」

另外提醒他：**分享粉專那則貼文，不要貼網址**；貼文設公開；**留言區不看診**。

## C. 三個平台的差別

|            | FB 貼文                  | IG 貼文                    | IG 限動                |
| ---------- | ------------------------ | -------------------------- | ---------------------- |
| 圖         | 方形 2048×2048           | 方形 2048×2048             | 直式 1080×1920         |
| 連結能不能點 | 內文可以但會降觸及；留言可以 | **都不能點**                 | 連結貼紙可以           |
| 連結放哪   | **第一則留言**             | 寫「連結在個人檔案」          | **連結貼紙**             |
| 文案長度   | 可以鋪陳，段落正常          | 短句、多換行，前兩行是鉤子     | 不放字，留白給貼紙       |
| hashtag    | 5～6 個                   | 15 個左右                   | 不用                   |

IG 留言的連結**一樣不能點**，所以 IG 沒有「放留言」這招。
IG 真正的點擊幾乎都來自限動的連結貼紙。

---

## D. 發布順序

- [ ] **FB**：貼方形圖 + FB 文案（內文不放連結）→ 自己在第一則留言貼文章連結
- [ ] **IG 貼文**：貼方形圖 + IG 文案（結尾「連結在個人檔案」）
- [ ] **IG 限動**：直式圖 → 貼紙 → 連結 → 貼上文章網址 →
      貼紙文字改成「看完整說明」→ 拖到圖片下方那塊空白 → 發布
- [ ] **限動存精選**：發布後點「更多 → 精選」，收進「衛教文章」，24 小時後還在
- [ ] **bio 連結**固定指到 https://rensin-clinic.sclin.net/
      （首頁就是文章列表，新文章自動排最上面，不必每篇改一次 bio）

---

## E0. 送出前的三道關卡

**關卡零管「能不能公開」，關卡一管「會不會出事」，關卡二管「有沒有你」。三關都過，才開始做素材。**

### 關卡零：這塊能不能公開（What/Why 還是 How）

煉金術 session 2026-09-13 補的一關，理由是前兩關都預設了「內容本來就可以公開」。
傑哥最強的東西——話術、分階、定價、篩選、診間的判斷機制——**一寫出來就破梗**，
而且有些根本不能讓病人或員工看到。

- **可以公開的是 What 與 Why**：這是什麼病、為什麼要處理、我們的原則是什麼。
- **不能公開的是 How**：怎麼讓病人願意自費、怎麼篩選、怎麼定價、復健的兩條規則、
  內部的縮編與轉型、`內部` 標記的金句（例如「不是樓上的人，是樓下的診」）。
- **破梗檢查**（煉金術 2026-09-13 交接）：凡是寫出**實際話術、判斷步驟、分級與篩選方法**的段落，
  都算 How，一律刪掉，只留結果與感受。那邊連學員回饋裡的「將病患分級、抓出大魚」都被傑哥兩個字打掉：「破梗」。
- **引用只能刪節，不能改寫**（見 memory 的 quote-only-trim）：患者原話、傑哥原話、別人的文章都一樣；
  要移除指名他人或不宜公開的部分，整句刪。唯一例外是療效誇大字眼，我主動修掉並回報。
- **提到別的做法時只講自己**，不貶低他人抬自己。傑哥的處理方式是把指控轉成理解——
  例如「這不是藏私，是常識」。
- 一句話版本（傑哥自己的）：**假設這件事，可不可以當著公開場合的面跟人家直接講？**
  答不出「可以」就別發。

### 關卡一：會不會出事（五問）

任何一題答「會」就不發：

1. 會不會傷害病人？
2. 會不會傷害同事？
3. 會不會傷害醫院／診所？
4. 會不會傷害自己的專業？
5. 十年後還願意看到這篇嗎？

等價的檢查：**假設這篇明天出現在法庭、新聞頭版或衛生局桌上，還願意送出嗎？**
社群沒有刪除鍵。細節見 memory 的「醫師社群生存原則」。

另外三條硬規則：**病人可被辨識的資訊一律不發**（年齡＋醫院＋日期＋特殊疾病拼得出來）、
**不評論正在發生的醫療事件**、**不公開批評同業**。

### 關卡二：有沒有你（三條）

法規限制的是不能亂宣稱，不是要所有醫師在網路上變成同一個人。
衛教內容誰講都一樣，換一個醫師來講也成立——那就不構成理由。

1. **觀點：這篇有沒有你的判斷，或你的拒絕？**
   可用的題型——「這幾種情況我反而不建議」「如果是我自己的家人，我會先做哪一項」
   「我什麼時候才會用震波」「我為什麼不接受指定檢查」。
   ⚠️ 只講我們的適應症判斷，**不點名同業、不點名產品、不說別人做得不對**。
2. **人：有沒有真實的場景與人？**
   診間、櫃檯、三樓走廊上遇到的人事物。衛教單張的語氣等於沒有人在說話。
   ⚠️ 「最討厭患者做什麼」改寫成「最讓我為難的」——**寫制度讓你為難，不寫人讓你討厭**。
3. **差異：換另一個骨科醫師講，這篇會不會一模一樣？**
   讀者看完知不知道**為什麼要找林醫師**？素材從 memory 的「我們跟別家的差異」清單挑，
   一律是他真的在做的事，不是形容詞。

---

## E1. 送出前問自己這三題（出自《打造超人大腦》的 onboarding 七問，取最痛的三題）

- [ ] **這次最容易忘記做、事後會讓我抓狂的是什麼？**
      （已經踩過的：封面圖少 logo、文章頁漏了 hero 圖、`?v=` 沒更新、草稿只改在對話裡沒落檔）
- [ ] **傑哥看到會問什麼？** 先想三個問題，能先答的就先改掉，不要等他問。
- [ ] **做錯了怎麼檢測、怎麼補救？** 部署完一定打實際網址驗證（帶 `?x=` 亂數避開快取），
      而不是看 CI 綠燈就當完成。

## E. 我每次發布完要交付的清單

文章上線之後，一次把下面這些列給傑哥，不要讓他自己去翻對話：

1. 文章網址（線上驗證過的）
2. FB 貼文文案
3. FB 第一則留言（文章連結）
4. FB／IG 貼文圖：`social-fb.png` 2048×2048
5. IG 貼文文案（含 hashtag）
6. IG 限動圖：`social-fb-story.png` 1080×1920
7. 限動連結貼紙要貼的網址
